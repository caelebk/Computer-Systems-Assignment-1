...
public class MainMemory extends AbstractMainMemory {
  ...
  @Override public int bytesToInteger (byte byteAtAddrPlus0, byte byteAtAddrPlus1, byte byteAtAddrPlus2, byte byteAtAddrPlus3) {
    // TODO
    return (byteAtAddrPlus0 << 24) + ((byteAtAddrPlus1 & 0xff) << 16) + ((byteAtAddrPlus2 & 0xff) << 8) + ((byteAtAddrPlus3 & 0xff) << 0);
  }
  ...
  @Override public byte[] integerToBytes (int i) {
    ...
    c[2] = (byte) (i >> 8);
    c[3] = (byte) (i >> 0);
    /*System.out.println(String.format("%x", c[0]));
    System.out.println(String.format("%x", c[1]));
    System.out.println(String.format("%x", c[2]));
    System.out.println(String.format("%x", c[3]));*/
  ...
  @Override protected byte[] get (int address, int length) throws InvalidAddressException {
    // TODO
      ...
      for(int x = 0; x < length; x++) {
          //java has negative addresses cuz signed
          if(mem.length < address) {
